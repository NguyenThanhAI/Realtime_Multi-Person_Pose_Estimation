...
def get_args():
    ...
    parser.add_argument("--dataset_dir", type=str, default="/data/mmaction/data/mmact/videos", help="Path to dataset")
    parser.add_argument("--output_dir", type=str, default="/data/caffe_mmact_annotator", help="Path to save json files")
    ...
    parser.add_argument("--video_save_dir", type=str, default="/data/caffe_mmact_save_videos")
...
if __name__ == '__main__':
    ...
    if args.run_format == "videos":
        ...
        for i in tqdm(range(start_index, len(videos_list))):
            ...
            param['scale_search'] = scale_search
            ...
            while (cam.isOpened()) and ret_val is True:
                ...
                if args.is_save_video:
                    # print("Visualize")
                    canvas = draw_canvas_with_info(orig_image, all_peaks, subset, candidate,
                                                   resize_fac=int(width / args.width))
                ...
                if video_class in check_id_classes:
                    if len(subset) > 0:
                        ...
                        predicted_coords = np.where(subset[:, :18, np.newaxis] > 0, candidate[:, :3][subset[:, :18]],
                                                    -1 * np.ones_like(subset[:, :18])[:, :, np.newaxis])
                        predicted_coords[:, :, :2] = np.where(predicted_coords[:, :, :2] > 0., (
                                    predicted_coords[:, :, :2] + np.array([1])[np.newaxis, np.newaxis, :]) / np.array(
                            [args.width, args.height])[np.newaxis, np.newaxis, :], predicted_coords[:, :, :2])
                        ...
                        if j == 0:
                            ...
                            dist_matrix = np.zeros(
                                shape=[len(predicted_reid_dict.keys()), len(previous_reid_dict.keys())])
                            for d, d_key in enumerate(predicted_reid_dict.keys()):
                                ...
                                for v, v_key in enumerate(previous_reid_dict.keys()):
                                    ...
                                    sub = np.where(np.logical_and(predicted_reid_dict[d_key][:, :2] > 0.,
                                                                  previous_reid_dict[v_key][:, :2] > 0), (
                                                               100 * predicted_reid_dict[d_key][:, :2] - 100 *
                                                               previous_reid_dict[v_key][:, :2]), np.zeros_like(
                                        predicted_reid_dict[d][:, :2]))  # Huhu, miss dkey->d, v_key->v and now fixed
                        ...
                        for k in predicted_reid_dict.keys():
                            frame_info["person_" + str(k).zfill(2)] = {k: v for k, v in zip(
                                list(map(lambda x: label_to_keypoint[x], list(range(predicted_reid_dict[k].shape[0])))),
                                list(map(lambda x: dict(zip(["x", "y", "prob"], list(map(lambda y: round(y, 3), x)))),
                                         predicted_reid_dict[k].tolist())))}
                    ...
                    for k, person in enumerate(subset):
                        ...
                        for h, point in enumerate(person):
                            if h == 18:
                                break
                            if point == -1.:
                                frame_info["person_" + str(k).zfill(2)][label_to_keypoint[h]] = {"x": -1.0, "y": -1.0,
                                                                                                 "prob": -1.0}
                                continue
                            x = int(candidate[point.astype(int), 0])
                            y = int(candidate[point.astype(int), 1])
                            prob = candidate[point.astype(int), 2]
                            frame_info["person_" + str(k).zfill(2)][label_to_keypoint[h]] = {
                                "x": round((x + 1) / args.width, 3),
                ...
                total = toc - tic
                print(
                    "Processing frame %d, processing time is %.5f, time rate of convert to rgb and resize  %.3f percent, source code to extract keypoints %.3f percent, write to dict %.3f percent" % (
                    j, total, ((tic_cvclars - tic) * 100. / total), ((tic_scekp - tic_cvclars) * 100. / total),
                    (toc - tic_scekp) * 100. / total))
                j += 1
                ret_val, orig_image = cam.read()
                # print(video_info)
                #
        ...
        for i in tqdm(range(start_index, len(videos_dict))):
            ...
            for j, image in enumerate(images_list):
                ...
                body_parts, all_peaks, subset, candidate = extract_parts(img_bgr, param, net, model)
                ...
                for k, person in enumerate(subset):
                    ...
                    for h, point in enumerate(person):
                        ...
                        prob = candidate[point.astype(int), 2]
                        frame_info["person_" + str(k).zfill(2)][label_to_keypoint[h]] = {
                            "x": round((x + 1) / args.width, 3), "y": round((y + 1) / args.height, 3),
                            "prob": round(prob, 3)}
